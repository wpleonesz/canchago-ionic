...
import AppInteractionAlert from '../feedback/AppInteractionAlert';
  ...
  const locateMe = async (): Promise<void> => {
    ...
    try {
      ...
      if (permission.location !== 'granted' && permission.coarseLocation !== 'granted') {
        setLocateError('No se concedió permiso de ubicación. Puedes marcar el punto manualmente en el mapa.');
        return;
      }
      const position = await Geolocation.getCurrentPosition();
      placeMarker(position.coords.latitude, position.coords.longitude);
      mapRef.current?.setView([position.coords.latitude, position.coords.longitude], SELECTED_ZOOM);
    } catch {
      setLocateError('No se pudo detectar tu ubicación. Puedes marcar el punto manualmente en el mapa.');
    } finally {
      setIsLocating(false);
    }
  };
    ...
    <IonModal isOpen={isOpen} className="location-picker-modal" onDidDismiss={onCancel}>
      ...
      <IonContent>
        ...
        <div className="location-picker-modal__actions">
          <AppButton fill="outline" isLoading={isLocating} onClick={() => void locateMe()}>
